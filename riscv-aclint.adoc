...
are tied to zero. The least significant bit always reads `0` but writing
`1` sends an edge-sensitive software interrupt signal to the corresponding
HART causing the HART to set SSIP bit in the `mip` CSR. Writing `0` to the
least significant bit of SETSSIP register has no effect. The machine-level
or supervisor-level software can directly clear the SSIP bit in `mip` or
`sip` CSR.
